...
myApp.toggle = (function($) {
  ...
  function _doToggle($toggle) {
    ...
    var selector = $toggle.data('target') === undefined ? 'toggle-target' : 'target';

    if($toggle.data('target') === 'parent') {
      $target = $($toggle.parent());

      if($toggle.data('parent-hop') !== undefined) {
        for (var i = 1; i < parseInt($toggle.data('parent-hop'), 10); i++) {
          $target = $target.parent();
        };
      }
    } else if ($toggle.data(selector) == 'next') {
      $target = $toggle.next();
    } else if ($toggle.data(selector) == 'previous') {
      $target = $toggle.prev();
    } else {
      $target = $($toggle.data(selector));
    }
  ...
  function _toggleIcon($toggle) {
    if($toggle.data('toggle-icon')) {
      ...
      var oldIcon = ($toggle.find('.js-icon').text());
      var newIcon = ($toggle.attr('data-toggle-icon'));

      $toggle.find('.js-icon').text(newIcon);
